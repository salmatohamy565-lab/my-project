...
BEGIN
    ...
    -- 1. Search across username, name, or email for candidates matching phone
    FOR v_candidate IN 
    ...
    LOOP
        ...
        -- If phone matches candidate record or candidate phone is empty
        IF (v_stored_phone_norm <> '' AND v_stored_phone_norm = v_input_phone_norm) OR
           (v_stored_phone_norm = '' AND v_input_phone_norm <> '') THEN
            v_matched_user := v_candidate;
            EXIT;
        END IF;
    END LOOP;

    -- 2. If no candidate matched phone, check if a candidate was locked or increment failed attempts on the primary candidate
...
with engine.connect() as conn:
    try:
        ...
        conn.commit()
        print("[SUCCESS] Deployed name/username/email matching passwordless_login RPC!")
