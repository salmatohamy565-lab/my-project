...
BEGIN
    ...
    -- 1. Check if user is a dedicated Staff member
    SELECT * INTO v_candidate FROM public.users 
    WHERE (LOWER(username) = LOWER(v_clean_username) OR LOWER(name) = LOWER(v_clean_username) OR LOWER(email) = LOWER(v_clean_username))
      AND (role IN ('admin', 'employee') OR LOWER(username) LIKE 'admin%' OR LOWER(username) LIKE 'emp%')
    ORDER BY id ASC LIMIT 1;

    IF FOUND THEN
        v_matched_user := v_candidate;
    ELSE
        -- 2. Strictly check word count for non-staff logins/registrations
        v_word_count := array_length(regexp_split_to_array(v_clean_username, '\s+'), 1);
        
        -- If not staff and name is fewer than 3 words (e.g. single name or 2 words)
        IF v_word_count < 3 THEN
            -- Check if it matches an existing staff account handle
            IF LOWER(v_clean_username) NOT IN ('admin', 'employee') AND 
               NOT (LOWER(v_clean_username) LIKE 'admin_%' OR LOWER(v_clean_username) LIKE 'emp_%') THEN
                RAISE EXCEPTION 'يرجى كتابة الاسم الثلاثي على الأقل (مثال: أحمد محمد محمود) لمنع اللخبطة وتكرار الحسابات';
            END IF;
        END IF;
    ...
    -- 3. If still no user matched, auto-register as customer with 3-word name
    IF v_matched_user IS NULL THEN
        v_email := LOWER(regexp_replace(v_clean_username, '\s+', '_', 'g')) || '@gmail.com';
    ...
    v_email := LOWER(COALESCE(v_matched_user.email, regexp_replace(v_clean_username, '\s+', '_', 'g') || '@gmail.com'));
...
with engine.connect() as conn:
    try:
        ...
        conn.commit()
        print("[SUCCESS] Deployed STRICT triple name requirement in Supabase PostgreSQL passwordless_login RPC!")
