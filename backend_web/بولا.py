...
def _perform_login(username_raw, password_raw=None, remember=False, phone_raw=None):
    clean_username = str(username_raw or '').strip()
    clean_phone = str(phone_raw or '').strip()
    clean_password = str(password_raw or '').strip() if password_raw else None

    if not clean_username:
        return jsonify({"error": "اسم المستخدم أو رقم الهاتف غير صحيح"}), 400

    user = find_user_by_identifier(clean_username)

    if not user:
        return jsonify({"error": "اسم المستخدم أو رقم الهاتف غير صحيح"}), 401

    if clean_phone:
        user_phone_norm = normalize_phone(user.phone)
        input_phone_norm = normalize_phone(clean_phone)

        if user_phone_norm and input_phone_norm:
            if user_phone_norm != input_phone_norm:
                return jsonify({"error": "اسم المستخدم أو رقم الهاتف غير صحيح"}), 401
        elif not user_phone_norm and input_phone_norm:
    ...
    record_login_attendance(user.id)
    return jsonify({"message": "تم تسجيل الدخول بنجاح", "user": user.to_dict(), "token": str(user.id)}), 200


@app.route('/api/customer/login', methods=['POST'])
@app.route('/api/customer/phone-login', methods=['POST'])
def customer_login():
    data = request.get_json() or {}
    return _perform_login(
        username_raw=data.get('username') or data.get('identifier') or data.get('name'),
